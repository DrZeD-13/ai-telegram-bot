...
final readonly class NativeModelsListMapper
{
    ...
    {
        $items = $payload['models'] ?? $payload['data'] ?? [];
        if (!is_array($items)) {
            throw new NeuralNetworkTransportException('Native models list payload is not an array.');
        }

        $models = [];
        foreach ($items as $item) {
            if (is_string($item) && $item !== '') {
                $models[] = $this->modelMapper->map(['id' => $item]);
                continue;
            }

            if (!is_array($item)) {
                throw new NeuralNetworkTransportException('Native models list contains a non-object item.');
            }
    ...
    private function stringKeyed(array $item): array
    {
        ...
        foreach ($item as $key => $value) {
            if (!is_string($key)) {
                throw new NeuralNetworkTransportException('Native models list item is not an object.');
            }
